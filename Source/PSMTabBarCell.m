...
- (void)setFrame:(NSRect)rect {
	...
	if(![[self indicator] isHidden] && ![(id)[self controlView] isTabBarHidden]) {
...
- (NSAttributedString *)attributedStringValue {
	return [(id < PSMTabStyle >)[(PSMTabBarControl*)(id)[self controlView] style] attributedStringValueForTabCell:self];
}
...
- (void)setIsInOverflowMenu:(BOOL)value {
	if(_isInOverflowMenu != value) {
		...
		if([[(id)[self controlView] delegate] respondsToSelector:@selector(tabView:tabViewItem:isInOverflowMenu:)]) {
            [[(id)[self controlView] delegate] tabView:(id)[self controlView] tabViewItem:[self representedObject] isInOverflowMenu:_isInOverflowMenu];
		}
	}
}
...
- (BOOL)closeButtonOver {
	return(_closeButtonOver && ([(id)[self controlView] allowsBackgroundTabClosing] || ([self tabState] & PSMTab_SelectedMask) || [[NSApp currentEvent] modifierFlags] & NSCommandKeyMask));
}
...
- (void)setHasIcon:(BOOL)value {
	_hasIcon = value;
	//[(id)[self controlView] update:(id)[self controlView]automaticallyAnimates]]; // binding notice is too fast
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
	...
	[(id)[self controlView] performSelector:@selector(update) withObject:nil afterDelay:0.0];
}
...
- (NSRect)indicatorRectForFrame:(NSRect)cellFrame {
	return [(id < PSMTabStyle >)[(PSMTabBarControl*)(id)[self controlView] style] indicatorRectForTabCell:self];
}

- (NSRect)closeButtonRectForFrame:(NSRect)cellFrame {
	return [(id < PSMTabStyle >)[(PSMTabBarControl*)(id)[self controlView] style] closeButtonRectForTabCell:self withFrame:cellFrame];
}

- (CGFloat)minimumWidthOfCell {
	return [(id < PSMTabStyle >)[(PSMTabBarControl*)(id)[self controlView] style] minimumWidthOfTabCell:self];
}

- (CGFloat)desiredWidthOfCell {
	return [(id < PSMTabStyle >)[(PSMTabBarControl*)(id)[self controlView] style] desiredWidthOfTabCell:self];
}
...
- (void)drawWithFrame:(NSRect)cellFrame inView:(NSView *)controlView {
	...
	[(id < PSMTabStyle >)[(PSMTabBarControl*)(id)[self controlView] style] drawTabCell:self];
}
...
- (void)mouseEntered:(NSEvent *)theEvent {
	...
	if([theEvent trackingNumber] == _cellTrackingTag) {
		...
		[(id)[self controlView] setNeedsDisplay:NO];
	}

	// scrubtastic
	if([(id)[self controlView] allowsScrubbing] && ([theEvent modifierFlags] & NSAlternateKeyMask)) {
		[(id)[self controlView] performSelector:@selector(tabClick:) withObject:self];
	}

	// tell the control we only need to redraw the affected tab
	[(id)[self controlView] setNeedsDisplayInRect:NSInsetRect([self frame], -2, -2)];
}
...
- (NSImage *)dragImage {
	NSRect cellFrame = [(id < PSMTabStyle >)[(PSMTabBarControl *)(id)[self controlView] style] dragRectForTabCell:self orientation:(PSMTabBarOrientation)[(PSMTabBarControl *)(id)[self controlView] orientation]];
	//NSRect cellFrame = [self frame];

	[(id)[self controlView] lockFocus];
	NSBitmapImageRep *rep = [[[NSBitmapImageRep alloc] initWithFocusedViewRect:cellFrame] autorelease];
	[(id)[self controlView] unlockFocus];
...
- (id)accessibilityAttributeValue:(NSString *)attribute {
	...
	if([attribute isEqualToString: NSAccessibilityRoleAttribute]) {
		...
		if([[(id)[self controlView] delegate] respondsToSelector:@selector(accessibilityStringForTabView:objectCount:)]) {
            
			attributeValue = [NSString stringWithFormat:@"%@, %lu %@", [self stringValue],
							  (unsigned long)[self count],
							  [[(id)[self controlView] delegate] accessibilityStringForTabView:[(id)[self controlView] tabView] objectCount:[self count]]];
		} else {
